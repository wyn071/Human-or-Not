...
const Masthead = () => {
  ...
  return (
    <header className="w-full mb-10">
      <div className="flex justify-between items-end text-[10px] font-mono uppercase tracking-[0.2em] mb-1 text-ink/70">
        <span>Late Edition</span>
        <span>Human or Not Press Bureau</span>
        <span>{today}</span>
      </div>
      
      <div className="border-y border-ink py-6 text-center space-y-4">
        <h1 className="text-7xl md:text-9xl font-serif font-black uppercase tracking-tighter leading-none ink-bleed">
          Human or Not
        </h1>
        <p className="text-[11px] md:text-sm font-serif uppercase tracking-[0.3em] font-medium opacity-80">
          A special newspaper game about spotting manufactured culture
        </p>
      </div>

      <div className="flex justify-between items-center text-[10px] font-mono uppercase tracking-[0.2em] py-2 border-b border-ink">
        <span>Vol. 1, No. 15</span>
        <span className="flex-1 text-center font-bold">Special Examination: Pre-AI Truth vs Machine Imitation</span>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen px-4 py-8 md:px-12 md:py-12 flex flex-col items-center relative">
      {/* Theme Toggle */}
      <div className="fixed top-4 right-4 z-[100] flex items-center gap-2 bg-paper/80 backdrop-blur-sm p-2 border border-ink/20 rounded-full shadow-sm">
        <button 
          onClick={() => setState(prev => ({ ...prev, theme: 'vintage' }))}
          className={`px-3 py-1 text-[10px] font-mono uppercase rounded-full transition-all ${state.theme === 'vintage' ? 'bg-ink text-paper' : 'hover:bg-ink/5'}`}
        >
          Vintage
        </button>
        <button 
          onClick={() => setState(prev => ({ ...prev, theme: 'modern' }))}
          className={`px-3 py-1 text-[10px] font-mono uppercase rounded-full transition-all ${state.theme === 'modern' ? 'bg-ink text-paper' : 'hover:bg-ink/5'}`}
        >
          Modern
        </button>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              <div className="md:col-span-2 space-y-8">
                <section className="space-y-4">
                  <h2 className="text-4xl font-serif font-bold uppercase border-b-2 border-ink inline-block pb-1 ink-bleed">
                    Front Page Investigation
                  </h2>
                  <h3 className="text-2xl font-serif italic text-news-red ink-bleed">
                    Can You Read the Difference?
                  </h3>
                  <p className="text-lg leading-relaxed text-ink/80 opacity-90 italic">
                    Some exhibits were made by real people before generative AI rewired our instincts. Others are modern fabrications crafted to sound or look authentic. Your job is to file the right verdict.
                  </p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm leading-relaxed tracking-tight text-ink/80 pt-4">
                    <p className="drop-cap">
                      Every round presents a single exhibit from the archive: a passage from literature, scholarship, public documents, or an image circulating as evidence. The challenge is not whether it is old, polished, or beautiful. The challenge is whether it was made by human intention or generated by a model imitating that intention.
                    </p>
                    <p>
                      The visual tone is deliberately editorial: worn paper, bureau notes, clipped metadata, and a front-page rhythm that makes each guess feel like a filed report. Strong instincts help, but so does noticing texture, specificity, weirdness, over-neat language, and the kind of detail machines often smooth away.
                    </p>
                  </div>
                </section>

                <div className="bg-ink text-paper p-8 space-y-6 relative overflow-hidden">
                   <div className="absolute top-2 right-4 opacity-50">
                    <div className="stamp !border-paper !text-paper">Confidential Archive</div>
                  </div>
                  <h4 className="text-xl font-mono uppercase tracking-widest border-b border-paper/30 pb-2">
                    Case Brief: Protocol 771-X
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-6">
                    <div className="space-y-2">
                      <h5 className="font-bold uppercase text-xs text-news-red mb-1">Goal</h5>
                      <p className="text-sm border-t border-dotted border-paper/30 pt-2">Decide whether each exhibit is <strong className="font-bold text-white">AI-generated</strong> or <strong className="font-bold text-white">Human-made.</strong></p>
                    </div>
                    <div className="space-y-2">
                      <h5 className="font-bold uppercase text-xs text-news-red mb-1">Format</h5>
                      <p className="text-sm border-t border-dotted border-paper/30 pt-2">Mixed media: literary excerpts, research-style prose, historical language, and archived images.</p>
                    </div>
                    <div className="space-y-2">
                      <h5 className="font-bold uppercase text-xs text-news-red mb-1">Scoring</h5>
                      <p className="text-sm border-t border-dotted border-paper/30 pt-2">10 points per correct verdict. Final rank is awarded like a newsroom promotion.</p>
                    </div>
                    <div className="space-y-2">
                      <h5 className="font-bold uppercase text-xs text-news-red mb-1">Tip</h5>
                      <div className="flex items-start gap-2 text-sm border-t border-dotted border-paper/30 pt-2">
                        <Keyboard size={14} className="mt-1" />
                        <span>Use the keyboard: press <strong className="font-bold text-white">A</strong> for AI, <strong className="font-bold text-white">H</strong> for Human, and <strong className="font-bold text-white">Enter</strong> for the next exhibit.</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <aside className="space-y-8 border-l-2 border-ink/10 pl-8">
                ...
                <div className="pt-4 border-t-2 border-ink">
                  <p className="text-[10px] font-mono uppercase mb-4 tracking-widest">
            ...
            <div className="relative">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="space-y-8"
                >
                  <div className="bg-white/50 p-1 shadow-sm border border-ink/10">
                    <div className="border-4 border-ink p-8 md:p-12 relative overflow-hidden bg-paper">
                      ...
                      <div className="max-w-2xl mx-auto space-y-6">
                        <div className="text-center space-y-2 mb-8">
                          <span className="text-xs font-mono uppercase tracking-[0.2em] text-news-blue font-bold">Evidence File #{currentExhibit.id}</span>
                          <h4 className="text-3xl font-serif font-bold leading-tight ink-bleed">{currentExhibit.headline}</h4>
                        </div>

                        {currentExhibit.type === 'text' ? (
                          <div className="text-xl md:text-2xl font-serif leading-relaxed italic text-ink/90 first-letter:text-5xl first-letter:font-bold first-letter:mr-2 first-letter:float-left ink-bleed">
                          ...
                          <div className="space-y-4">
                            <img 
                              ...
                              alt="Evidence" 
                              className="w-full h-auto grayscale contrast-125 hover:grayscale-0 transition-all duration-700 border-2 border-ink"
                              referrerPolicy="no-referrer"
                            />
                            <p className="text-xs font-mono uppercase text-center text-news-grey italic">
        ...
        {state.screen === 'results' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="bg-paper p-8 border-2 border-ink shadow-xl space-y-6">
                <h4 className="text-xl font-serif font-bold border-b-2 border-ink pb-2 ink-bleed">Statistical Breakdown</h4>
                <div className="space-y-4">
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs font-mono uppercase text-news-grey">
